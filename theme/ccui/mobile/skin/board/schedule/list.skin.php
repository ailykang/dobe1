...
$query = "SELECT * FROM $write_table WHERE left(wr_3,6) = '$year$sel_mon' ORDER BY wr_id ASC";
//echo $query;
$result = sql_query($query);
$j=0; // layer id
// 내용을 보여주는 부분
while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
  if( substr($row[wr_3],0,6) >  $year.$sel_mon ) {
	 $start_day = $lastday[$month];
	 $start_day= (int)$start_day;
  } else {
	 $start_day = substr($row[wr_3],6,2);
	 $start_day= (int)$start_day;
  }
  ...
  for ($i = $start_day ; $i <= $end_day;  $i++) {
    ...
    $html_day[$i].= "<br /> <a href='".G5_BBS_URL."/board.php?bo_table=$bo_table&year=$year&month=$month&wr_id=$row[wr_id]&sc_no=$sc_no' id='subject_".$j."' ".$showLayer.">".$row[wr_1]."님의 견적문의</a>".$list[icon_new].$list[comment_cnt];
	?>
    <!-- 뷰 팝업레이어 -->
...
<?php
    ...
    if (strstr($row[wr_option], "html1"))
	  ...
	  echo "( 신청인 : ".$row[wr_1]." )<br />";
      //echo $viewlist;
?>
    </DIV>
<?php
		//오늘 스케줄 구하기
		if ($row[wr_id] != $sc_id && date('Ymd', strtotime($row[wr_1])) <= date(Ymd) && date('Ymd', strtotime($row[wr_3])) >= date(Ymd)) {
			$today_schedule .= "<p><a href='".G5_BBS_URL."/board.php?bo_table=$bo_table&year=$year&month=$month&wr_id=$row[wr_id]&sc_no=$sc_no'><b>".$row[wr_subject]."</b></a>";
			$today_schedule .= "<br />";
...
<table cellpadding='0' cellspacing='0' width="100%" style="border:1px solid #CCC;">
	<tr style="border-bottom:1px solid #CCC;" align="center">
		...
		<td style="background-color:#EFEFEF;border-right:1px solid #CCC;"><b><?php echo $t; ?> 견적문의 목록</td>
		<td style="background-color:#EFEFEF;border-right:1px solid #CCC;"><b>시공희망날짜</td>
		<td style="background-color:#EFEFEF;border-right:1px solid #CCC;"><b>고객연락처</td>
	</tr>
	<?php
		//$sql = " select wr_subject, wr_5, wr_link2, wr_id, wr_3 from g5_write_{$bo_table} where wr_1 = '{$t}' or (wr_1 <= '{$t}' and wr_3 >= '{$t}') order by wr_id desc ";	
	...
	<tr>
		...
		<td style="border-right:1px solid #CCC;border-top:1px solid #CCC;padding:5px;" align="center"><a href="<?php echo G5_BBS_URL;?>/board.php?bo_table=<?php echo $bo_table;?>&wr_id=<?php echo $row[wr_id];?>"><?php echo $row[wr_1]; ?>님의 <?php echo $row[wr_subject]; ?></a>
		<td style="border-right:1px solid #CCC;border-top:1px solid #CCC;" height="35" align="center"><?php echo $row[wr_3]; ?></td>
		<td style="border-right:1px solid #CCC;border-top:1px solid #CCC;" height="35" align="center"><?php echo $row[wr_2]; ?></td>
		<br>
		<!--
